...
void wave_2d_verify(long TSTEPS, long N, long M, long ts1, long ts2, long ts3, double** _local_B){
	///Parameter checking
	if (!((TSTEPS >= 3 && N >= 7 && M >= 7))) {
	...
	{
		//Domain
		//{i,j|i>=0 && N>=i+1 && j>=0 && M>=j+1 && TSTEPS>=3 && N>=7 && M>=7}
		int c1,c2;
...
double eval_temp_B(long TSTEPS, long N, long M, int t, int i, int j){
	...
	//Body for temp_B
		temp_B(t,i,j) = (((t == 0))?(((i)*(i))+((j)*(j)))/(1024):(((t >= 1 && j >= 3 && i >= 3 && N >= i+4 && M >= j+4))?(0.077)*(((((((((((((eval_temp_B(TSTEPS,N,M,t-1,i-3,j))+(eval_temp_B(TSTEPS,N,M,t-1,i-2,j)))+(eval_temp_B(TSTEPS,N,M,t-1,i-1,j)))+(eval_temp_B(TSTEPS,N,M,t-1,i+3,j)))+(eval_temp_B(TSTEPS,N,M,t-1,i+2,j)))+(eval_temp_B(TSTEPS,N,M,t-1,i+1,j)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j-3)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j-2)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j-1)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j+3)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j+2)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j+1)))+(eval_temp_B(TSTEPS,N,M,t-1,i,j))):(eval_temp_B(TSTEPS,N,M,t-1,i,j))));
