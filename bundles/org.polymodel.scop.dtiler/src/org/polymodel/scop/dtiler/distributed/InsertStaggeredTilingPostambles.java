...
tom_make_BottomUp(tom_make_findTargetLoops(statementName,factor,numberOfDim)).visitLight(root, tom.mapping.GenericIntrospector.INSTANCE);
...
ScopFor postamble = (ScopFor)
tom_make_loop(firstLoop.getIterator(),LB,UB,firstLoop.getStride().copy(),tom_make_block(tom_cons_array_nodes(stmt,tom_empty_array_nodes(1))));
...
public static class findTargetLoops extends tom.library.sl.AbstractStrategyBasic {
private  String  stmtName;
private  int  factor;
private  int  numberOfDim;
public findTargetLoops( String  stmtName,  int  factor,  int  numberOfDim) {
...
AbstractScopNode tom_stmt=((AbstractScopNode)tom__arg);

if (
tom_get_slot_stmtMacro_name(((AbstractScopNode)tom__arg)).contentEquals(stmtName)) {
EList<ScopFor> loops = ((ScopStatementMacro)
tom_stmt).getSurroundingScopFors();
if (loops.size()>0) {
ScopFor targetLoop = loops.get(loops.size()-1);

int depth = 
tom_stmt.getLoopDepth();
if (statementDepth == -1) {
...
private static  tom.library.sl.Strategy  tom_make_findTargetLoops( String  t0,  int  t1,  int  t2) { 
return new findTargetLoops(t0,t1,t2);
}
