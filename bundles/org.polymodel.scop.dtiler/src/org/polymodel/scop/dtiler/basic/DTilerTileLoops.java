...
try {
ScopFor res = (ScopFor)
tom_make_BottomUp(tom_make_TileLoopStrategy(options)).visitLight(EcoreUtil.copy(perfectNest),tom.mapping.introspectors.scop.ScopIntrospector.INSTANCE);
...
public static class TileLoopStrategy extends tom.library.sl.AbstractStrategyBasic {
private DTilingOptions options;
public TileLoopStrategy(DTilingOptions options) {
...
AbstractScopNode tileLoop = 
tom_make_loop(tileIndex,lbe.simplify(),ube.simplify(),tileSize,tom_get_slot_loop_body(((AbstractScopNode)tom__arg)));
//add tiled loop annotations to save the tiled loop informations
tileLoop.getScopAnnotations().add(ScopUserFactory.tileLoopAnnotation(
tileIndex, 
tileSize, options.tilingBand.getOffset(depth)));
...
private static  tom.library.sl.Strategy  tom_make_TileLoopStrategy(DTilingOptions t0) { 
return new TileLoopStrategy(t0);
}
