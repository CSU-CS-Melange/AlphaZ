...
tom_make_BottomUp(tom_make_replace(reduceVar,originalVar,projection)).visitLight(system, tom.mapping.GenericIntrospector.INSTANCE);
...
private VariableDeclaration redVar;
private VariableDeclaration origVar;
private AffineFunction proj;
public replace(VariableDeclaration redVar, VariableDeclaration origVar, AffineFunction proj) {
super(tom_make_Identity());
this.redVar=redVar;
this.origVar=origVar;
this.proj=proj;
}
public VariableDeclaration getredVar() {
return redVar;
}
public VariableDeclaration getorigVar() {
return origVar;
}
public AffineFunction getproj() {
return proj;
}
...
Expression tomMatch1_2=tom_get_slot_depExpr_expr(((Expression)tom__arg));
if (tom_is_sort_Expression(tomMatch1_2)) {
if (tom_is_fun_sym_var(((Expression)tomMatch1_2))) {

if (
tom_get_slot_var_varDecl(tomMatch1_2).equals(origVar)) {
return 
tom_make_depExpr(proj.compose(tom_get_slot_depExpr_dep(((Expression)tom__arg))),tom_make_var(redVar));
} else {
return 
((Expression)tom__arg);
}
...
private static  tom.library.sl.Strategy  tom_make_replace(VariableDeclaration t0, VariableDeclaration t1, AffineFunction t2) { 
return new replace(t0,t1,t2);
}
