...
(JNIEnv *env, jclass class, jlong expr)
 {
#ifdef TRACE_ALL
	printf("Entering isl_ast_expr_get_val\n");fflush(stdout);
#endif
	struct isl_ast_expr* expr_c = (struct isl_ast_expr*) GECOS_PTRSIZE expr; 
	if(((void*)expr_c)==NULL) {
		throwException(env, "Null pointer in isl_ast_expr_get_val for parameter expr");
		goto error;
	}

	isl_val* res = (isl_val*) isl_ast_expr_get_val(expr_c);


#ifdef TRACE_ALL
	printf("Leaving isl_ast_expr_get_val\n");fflush(stdout);
#endif
...
(JNIEnv *env, jclass class, jlong expr)
 {
#ifdef TRACE_ALL
	printf("Entering isl_ast_expr_is_literal\n");fflush(stdout);
#endif
	struct isl_ast_expr* expr_c = (struct isl_ast_expr*) GECOS_PTRSIZE expr; 
	if(((void*)expr_c)==NULL) {
		throwException(env, "Null pointer in isl_ast_expr_is_literal for parameter expr");
		goto error;
	}

	int res = (int) isl_ast_expr_is_literal(expr_c);


#ifdef TRACE_ALL
	printf("Leaving isl_ast_expr_is_literal\n");fflush(stdout);
#endif
