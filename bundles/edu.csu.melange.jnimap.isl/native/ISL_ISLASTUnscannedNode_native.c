...
(JNIEnv *env, jclass class, jlong node)
 {
#ifdef TRACE_ALL
	printf("Entering isl_ast_node_user_get_schedule\n");fflush(stdout);
#endif
	struct isl_ast_node* node_c = (struct isl_ast_node*) GECOS_PTRSIZE node; 
	if(((void*)node_c)==NULL) {
		throwException(env, "Null pointer in isl_ast_node_user_get_schedule for parameter node");
		goto error;
	}
...
#ifdef TRACE_ALL
	printf("Leaving isl_ast_node_user_get_schedule\n");fflush(stdout);
#endif
...
(JNIEnv *env, jclass class, jlong node)
 {
#ifdef TRACE_ALL
	printf("Entering isl_ast_node_is_unscanned\n");fflush(stdout);
#endif
	struct isl_ast_node* node_c = (struct isl_ast_node*) GECOS_PTRSIZE node; 
	if(((void*)node_c)==NULL) {
		throwException(env, "Null pointer in isl_ast_node_is_unscanned for parameter node");
		goto error;
	}
...
#ifdef TRACE_ALL
	printf("Leaving isl_ast_node_is_unscanned\n");fflush(stdout);
#endif
