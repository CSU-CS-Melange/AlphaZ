...
(JNIEnv *env, jclass class, jlong pwa, jint format)
 {
#ifdef TRACE_ALL
	printf("Entering isl_pw_aff_to_string\n");fflush(stdout);
#endif
	isl_pw_aff* pwa_c = (isl_pw_aff*) GECOS_PTRSIZE pwa; 
	if(((void*)pwa_c)==NULL) {
		throwException(env, "Null pointer in isl_pw_aff_to_string for parameter pwa");
		goto error;
	}
...
#ifdef TRACE_ALL
	printf("Leaving isl_pw_aff_to_string\n");fflush(stdout);
#endif
