...
(JNIEnv *env, jclass class, jlong node)
 {
#ifdef TRACE_ALL
	printf("Entering isl_ast_node_user_get_expr\n");fflush(stdout);
#endif
	struct isl_ast_node* node_c = (struct isl_ast_node*) GECOS_PTRSIZE node; 
	if(((void*)node_c)==NULL) {
		throwException(env, "Null pointer in isl_ast_node_user_get_expr for parameter node");
		goto error;
	}

	struct isl_ast_expr* res = (struct isl_ast_expr*) isl_ast_node_user_get_expr(node_c);


#ifdef TRACE_ALL
	printf("Leaving isl_ast_node_user_get_expr\n");fflush(stdout);
#endif
...
(JNIEnv *env, jclass class, jlong node)
 {
#ifdef TRACE_ALL
	printf("Entering isl_ast_node_is_user\n");fflush(stdout);
#endif
	struct isl_ast_node* node_c = (struct isl_ast_node*) GECOS_PTRSIZE node; 
	if(((void*)node_c)==NULL) {
		throwException(env, "Null pointer in isl_ast_node_is_user for parameter node");
		goto error;
	}
...
#ifdef TRACE_ALL
	printf("Leaving isl_ast_node_is_user\n");fflush(stdout);
#endif
