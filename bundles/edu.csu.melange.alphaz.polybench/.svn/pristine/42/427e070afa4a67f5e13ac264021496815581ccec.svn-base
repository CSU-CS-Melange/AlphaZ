prog = ReadAlphabets("../../../../polybench-alpha-4.0/linear-algebra/kernels/2mm/2mm.ab");
system = "2mm";
outDir = "../../../../test-out/DTiler/"+system;

CheckProgram(prog);

#PrintAST(prog, system);

# Name of the new variable: "_ser" + (varName of container equation)
SerializeReduction(prog, "(0,0,0,1)", "(i,j,k-> i,j,k)");    # => _sertemp
SerializeReduction(prog, "(0,1,0,1)", "(i,j,k-> i,j,k)");    # => _serE
Simplify(prog, system);

#AShow(prog, system);


# Schedule
setSpaceTimeMap(prog, system, "_sertemp", "(i,j,k -> 0, i, j, 0, k)");
setSpaceTimeMap(prog, system, "temp",     "(i,j   -> 0, i, j, 1, 0)");
setSpaceTimeMap(prog, system, "_serE",    "(i,j,k -> 1, i, j, 0, k)");
setSpaceTimeMap(prog, system, "E",        "(i,j   -> 1, i, j, 1, 0)");
setOrderingDimensions(prog, system, "0,3");


# Memory map
setMemorySpace(prog, system, "temp", "_sertemp, temp");
setMemoryMap(prog, system, "_sertemp", "temp", "(i,j,k->i,j)");
setMemoryMap(prog, system, "temp", "temp", "(i,j->i,j)");

setMemorySpace(prog, system, "E", "_serE, E");
setMemoryMap(prog, system, "_serE", "E", "(i,j,k->i,j)");
setMemoryMap(prog, system, "E", "E", "(i,j->i,j)");

#tile the whole program
options = createTiledCGOptionForScheduledC();
setDefaultDTilerConfiguration(prog, system, "sequential");


# Code Generation
generateScheduledCode(prog, system, options, outDir);
generateWrapper(prog, system, options, outDir);
generateMakefile(prog, system, outDir);

