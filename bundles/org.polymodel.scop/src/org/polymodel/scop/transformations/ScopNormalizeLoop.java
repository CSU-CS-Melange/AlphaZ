...
try {
return 
tom_make_BottomUp(tom_make_NormalizeLoop()).visitLight(root.simplify(), tom.mapping.GenericIntrospector.INSTANCE);
...
try {
return 
tom_make_BottomUp(tom_make_NormalizeLoop()).visitLight(node.simplify(), tom.mapping.GenericIntrospector.INSTANCE);
} catch (VisitFailure vf) {
vf.printStackTrace();
throw new RuntimeException("Visit Failure " + vf.getMessage());
}
...
public static class NormalizeLoop extends tom.library.sl.AbstractStrategyBasic {
public NormalizeLoop() {
...
IntExpression tom_ub=tom_get_slot_loop_UB(((AbstractScopNode)tom__arg));
IntExpression tom_stride=tom_get_slot_loop_stride(((AbstractScopNode)tom__arg));
AbstractScopNode tom_body=tom_get_slot_loop_body(((AbstractScopNode)tom__arg));

if (!((
tom_body) instanceof ScopBlock)) {
if (
tom_body== null) {
return 
tom_make_loop(tom_ite,tom_lb,tom_ub,tom_stride,tom_make_block(tom_empty_array_nodes(0)));
} else {
return 
tom_make_loop(tom_ite,tom_lb,tom_ub,tom_stride,tom_make_block(tom_cons_array_nodes(tom_body,tom_empty_array_nodes(1))));
}
} else {
return 
((AbstractScopNode)tom__arg);
}
...
private static  tom.library.sl.Strategy  tom_make_NormalizeLoop() { 
return new NormalizeLoop();
}
